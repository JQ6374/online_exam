  ...
  >
    <template #main>
      <el-table width="100%" :height=useMainHeight().mainHeight :data="tableData" stripe>
        <el-table-column align="center" prop="pId" label="试卷号" fixed/>
      ...
      <!-- 提示框 -->
      <el-dialog v-model="dialogFormVisible" title="修改试卷数据">
        <el-form :model="form">
          <el-form-item label="试卷号" :label-width="formLabelWidth">
...
const getTableData = async () => {
  await myRequest.get<any, ApiResult>(`/papers/${userStore.uId}`)
      .then(res => {
...
const updateName = async () => {
  ...
  try {
    const res = await request.put<any, ApiResult>(`/papers/${form.pId}/${form.name}`)
...
const handleDelete = (row) => {
  ElMessageBox.confirm('此操作将永久删除数据, 是否继续?', '提示', {
    ...
    const res = await request.delete<any, ApiResult>(`/papers/${row.pId}`)
